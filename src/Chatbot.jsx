...
// Imports React and useState for the chatbot component
import React, { useState } from "react";

// Hardcoded list of AI models users can choose from
function Chatbot() {
    const API_BASE = import.meta.env.VITE_API_URL || "/api";
    const models = ["llama3.2:latest", "llama3.2"];
    ...
    const send = async () => {
      ...
      try {
        ...
        const data = await res.json(); // Parses the backend JSON response into a JavaScript object
        // Extracts the AI's message content and stores it in state, falls back to any error string if missing
        setResponse(
          data.message?.content ||
            data.error ||
            "No response came back from the chat server."
        );
      } catch {
        setResponse(
          "Unable to reach the chat server. Start the backend with `npm run dev:server`, then make sure Ollama is running."
        );
      } finally {
        setLoading(false);
      }
    ...
    <div className = "flex flex-col items-center justify-center min-h-screen">
      ...
      <div className="flex flex-col gap-3 mt-3 min-h-12">
    ...
    <input className="flex-1 p-3 w-70 rounded-md border-zinc-500 bg-zinc-600 text-white" type="text" value={text} onChange={(e) => {setText(e.target.value)}}/>
      ...
      <button className="cursor-pointer ml-3 mt-3 px-4 py-2 bg-orange-500 hover:bg-orange-700 text-white rounded-md disabled:cursor-not-allowed disabled:bg-orange-300" disabled={loading} onClick={send}>
